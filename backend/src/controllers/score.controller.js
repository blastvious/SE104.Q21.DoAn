...
export const getScore = async (req, res) => {
    try {
        const {MaLop, MaMonHoc, MaHocKy, MaHS, MaLoaiHinhKT, Lan, Diem} = req.body;

        if (!MaLop || !MaMonHoc || !MaHocKy || !MaHS || !MaLoaiHinhKT || !Lan || Diem === undefined) {
        ...
        const [ctBangDiemMonLHKT, created] = await db.CT_BANGDIEMMON_LHKT.findOrCreate({
            where: {
...
export const bulkImportScores = async (req, res) => {
    try {
        const {MaLop, MaMonHoc, MaHocKy, MaLoaiHinhKT, Lan, danhSachDiem} = req.body;

        if (!MaLop || !MaMonHoc || !MaHocKy || !MaLoaiHinhKT || !Lan || !danhSachDiem?.length) {
...
export const getBangDiemMon = async(req, res) => {
    try {
        ...
        if (!bangDiemMon){
            return res.status(404).json({message: "The grade sheet does not exist."});
        }
