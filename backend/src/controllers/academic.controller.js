...
export const createSubject = async (req, res) => {
    try {
        ...
        const lastSubject = await db.MONHOC.findOne({
            ...
            order: [
                // Sắp xếp theo phần số sau 'MH', ép kiểu sang số nguyên
                [db.Sequelize.literal("CAST(SUBSTRING(MaMonHoc, 2) AS INT)"), "DESC"]
            ]
        });
...
export const createExamType = async (req, res) => {
    try {
        ...
        const lastExamType = await db.LOAIHINHKT.findOne({
            ...
            order: [
                // Sắp xếp theo phần số sau 'LHLT', ép kiểu sang số nguyên
                [db.Sequelize.literal("CAST(SUBSTRING(MaLoaiHinhKT, 5) AS INT)"), "DESC"]
            ]
        });
